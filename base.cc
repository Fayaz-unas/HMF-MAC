...
BaseCache::BaseCache(const BaseCacheParams &p, unsigned blk_size)
    : ClockedObject(p),
      ...
      system(p.system),
      stats(*this),
      adaptive(),          // ✅ Correctly initialized
      is_adaptive(false)
{
...
BaseCache::recvTimingReq(PacketPtr pkt)
{
    ...
    if (satisfied) {
        ...
        MSHR *mshr = mshrQueue.findMatch(pkt->getBlockAddr(blkSize), pkt->isSecure());
        handleTimingReqMiss(pkt, mshr, blk, forward_time, request_time);
        ppMiss->notify(pkt);
    }
    if (prefetcher) {
                  ...
                  PacketList &writebacks)
{
    ...
    bool is_hit = (blk != nullptr);
    bool is_conflict = false;

    if (is_adaptive) {
        is_conflict = (!is_hit && blk == nullptr);
        adaptive.update(pkt->getAddr(), is_hit, is_conflict);
    }
...
BaseCache::allocateWriteBuffer(PacketPtr pkt, Tick time)
{
    ...
    // Allocate directly since this version of WriteQueueEntry 
    // doesn't use addTarget/allocateTarget coalescing
    writeBuffer.allocate(pkt->getBlockAddr(blkSize), blkSize, pkt, time, order++);
}
void
BaseCache::writebackTempBlockAtomic()
{
    assert(tempBlockWriteback);
    // writeback the temporary block
    memSidePort.sendAtomic(tempBlockWriteback);
    delete tempBlockWriteback;
    tempBlockWriteback = nullptr;
}
} // namespace gem5